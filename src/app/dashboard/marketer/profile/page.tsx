...
const REQUIRED_FIELDS_MARKETER: (keyof MarketerData)[] = [
  'full_name', 'phone', 'email', 'city', 'country', 'specialization', 'bio',
];
...
export default function MarketerProfilePage() {
  ...
  const copy = getTranslations<any>('marketerProfile');
  ...
  const clientLabel = copy.clients[marketerData.client_type];
  const clientsLabel = marketerData.client_type === 'trainer' ? copy.clients.trainers : copy.clients.players;
  ...
  const validateForm = (): boolean => {
    ...
    REQUIRED_FIELDS_MARKETER.forEach((key) => {
      const val = marketerData[key];
      if (!val || (typeof val === 'string' && !val.trim())) {
        newErrors[key] = interpolate(copy.required, { field: copy.requiredFields[key] });
      }
    });
    setErrors(newErrors);
  ...
  const handleAddDeal = () => {
    if (!newDeal.client_name.trim()) {
      toast.error(interpolate(copy.enterClientName, { client: clientLabel }));
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-indigo-50" dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Sticky Header */}
      ...
      <div className="px-4 py-8 mx-auto max-w-4xl">
        ...
        {showCompletionBanner && !editMode && (
          <div className="flex gap-4 items-start p-4 mb-6 bg-amber-50 rounded-xl border-2 border-amber-300 shadow-sm">
            ...
            <div className="flex-1">
              <p className="mb-1 text-sm font-bold text-amber-800">{copy.bannerTitle}</p>
              <p className="mb-2 text-xs text-amber-700">{copy.bannerDescription}</p>
              <div className="flex flex-wrap gap-1.5 mb-3">
                {missingFields.map((key) => (
                  <span key={key} className="px-2 py-0.5 text-xs font-medium text-amber-700 bg-amber-100 rounded-full border border-amber-300">
                    {copy.requiredFields[key]}
                  </span>
                ))}
              </div>
              <div className="flex gap-3 items-center">
                ...
                <span className="text-xs text-amber-500">{interpolate(copy.snooze, { days: SNOOZE_DAYS })}</span>
              </div>
            </div>
            <button
        ...
        <div className="flex flex-col gap-6 items-center p-8 mb-8 bg-white rounded-2xl shadow-lg md:flex-row">
          ...
          <div className="flex-1 text-right">
            {editMode ? (
              ...
              <h2 className="mb-1 text-2xl font-bold text-indigo-700">{marketerData.full_name || copy.defaultName}</h2>
            )}
            ...
            {editMode ? (
              ...
              <p className="mb-3 text-gray-600">{marketerData.specialization || copy.defaultSpecialization}</p>
            )}
              ...
              <span className="flex gap-1 items-center">
                ...
                {editMode ? (
                  <input type="text" value={marketerData.city} onChange={e => { handleChange('city', e.target.value); setErrors(p => ({ ...p, city: '' })); }}
                    placeholder={copy.city} className={`w-20 bg-transparent border-b focus:outline-none ${errors.city ? 'border-red-400' : 'border-gray-200'}`} />
                ) : (marketerData.city || '—')}
                {editMode ? (
                  <input type="text" value={marketerData.country} onChange={e => { handleChange('country', e.target.value); setErrors(p => ({ ...p, country: '' })); }}
                    placeholder={copy.country} className={`w-20 bg-transparent border-b focus:outline-none ${errors.country ? 'border-red-400' : 'border-gray-200'}`} />
              ...
              <span className="flex gap-1 items-center">
                ...
                {editMode ? (
                  <input type="number" value={marketerData.experience_years} onChange={e => handleChange('experience_years', e.target.value)}
                    placeholder={copy.years} className="w-12 bg-transparent border-b border-gray-200 focus:outline-none" />
                ) : marketerData.experience_years} {copy.experienceYears}
              </span>
            </div>

            {/* Client Type Toggle */}
            <div className="flex gap-2 mt-4">
              <span className="self-center text-sm text-gray-500">{copy.clientType}</span>
              <button
                disabled={!editMode}
        ...
        <div className="grid grid-cols-2 gap-4 mb-8 md:grid-cols-4">
          {[
            ...
            { icon: <Handshake size={26} />, label: copy.stats.activeDeals, field: 'active_deals', color: 'from-green-400 to-green-600' },
            { icon: <Trophy size={26} />, label: copy.stats.completedDeals, field: 'completed_deals', color: 'from-yellow-400 to-yellow-600' },
            { icon: <Star size={26} />, label: copy.stats.rating, field: 'rating', color: 'from-purple-400 to-purple-600' },
        ...
        <div className="grid grid-cols-1 gap-8 mb-8 md:grid-cols-2">
          ...
          <div className="p-6 bg-white rounded-xl shadow">
            ...
            <div className="space-y-3">
              {[
                { icon: <Phone size={15} />, label: copy.contact.phone, field: 'phone', type: 'tel' },
                { icon: <Phone size={15} />, label: copy.contact.whatsapp, field: 'whatsapp', type: 'tel' },
                { icon: <Mail size={15} />, label: copy.contact.email, field: 'email', type: 'email' },
                { icon: <MapPin size={15} />, label: copy.contact.nationality, field: 'nationality', type: 'text' },
                { icon: <Shield size={15} />, label: copy.contact.license, field: 'license_number', type: 'text' },
        ...
        <div className="p-6 mb-8 bg-white rounded-xl shadow">
          <div className="flex justify-between items-center mb-5">
            <h3 className="flex gap-2 items-center text-lg font-bold text-indigo-700">
              <Handshake size={20} /> {copy.dealsTitle}
            </h3>
            {editMode && (
              <button
          ...
          {showAddDeal && (
            <div className="p-4 mb-5 rounded-xl border border-indigo-100 bg-indigo-50">
              <h4 className="mb-3 font-semibold text-indigo-700">{copy.newDeal}</h4>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div>
                  <label className="block mb-1 text-xs text-gray-600">{copy.clientType}</label>
                  <div className="flex gap-2">
                ...
                {[
                  { label: interpolate(copy.clientName, { client: copy.clients[newDeal.client_type] }), field: 'client_name' },
                  { label: copy.previousClub, field: 'from_club' },
                  { label: copy.newClub, field: 'to_club' },
                  { label: copy.dealValue, field: 'deal_value' },
                  { label: copy.season, field: 'season' },
                  { label: copy.year, field: 'year' },
                ].map(({ label, field }) => (
                  <div key={field}>
                ...
                <div className="md:col-span-2">
                  <label className="block mb-1 text-xs text-gray-600">{copy.notes}</label>
          ...
          {marketerData.deals.length === 0 ? (
            <div className="py-10 text-center text-gray-400">
              ...
              <p className="text-sm">{copy.noDeals}</p>
            </div>
          ) : (
            <div className="space-y-3">
        ...
        <div className="p-6 mb-8 bg-white rounded-xl shadow">
          <h3 className="flex gap-2 items-center mb-5 text-lg font-bold text-indigo-700">
            <Globe size={20} /> {copy.socialTitle}
          </h3>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {[
              ...
              { icon: <Globe size={16} className="text-indigo-600" />, label: copy.website, field: 'website' },
        ...
        <div className="p-6 bg-white rounded-xl shadow">
          <h3 className="flex gap-2 items-center mb-5 text-lg font-bold text-indigo-700">
            <Camera size={20} /> {copy.galleryTitle}
          </h3>
