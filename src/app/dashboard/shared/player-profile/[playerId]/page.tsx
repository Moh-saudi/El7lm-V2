...
function PlayerReportPage() {
  ...
  const { locale, getTranslations, t } = useTranslation();
  const copy = getTranslations<any>('playerProfile');
  useEffect(() => {
        ...
        <div className="bg-teal-50 rounded-lg border border-teal-100 p-4">
          ...
          {player?.languages && player.languages.length > 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {player?.english_level && <div className="bg-white p-3 rounded shadow-sm">🇺🇸 {t('sharedComponents.playerProfile.english')}: {player.english_level}</div>}
            ...
            <div className="bg-gray-50 rounded-lg p-4 text-center">
              ...
              {bmi && <div className={`text-xs mt-1 font-medium ${Number(bmi) < 18.5 ? 'text-blue-600' :
                Number(bmi) < 25 ? 'text-green-600' :
                  'text-orange-600'
                }`}>{t('sharedComponents.playerProfile.bmi')}: {bmi}</div>}
            </div>
            <div className="bg-gray-50 rounded-lg p-4 text-center">
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            {t('sharedComponents.playerProfile.openInNewWindow')}
                          </a>
            ...
            <div className="p-8 max-w-md text-center bg-white rounded-lg shadow-md">
              ...
              <div className="p-3 mb-4 text-xs text-left bg-gray-50 rounded-lg">
                <div className="font-mono">
                  <div>🔍 {t('sharedComponents.playerProfile.playerId')}: {targetPlayerId || copy.common.notSpecified}</div>
                  <div>👤 {t('sharedComponents.playerProfile.userId')}: {user?.id || copy.common.notSpecified}</div>
                  <div>🔗 {t('sharedComponents.playerProfile.viewMode')}: {playerIdFromUrl ? copy.sections.detailedPlayerProfile : copy.sections.tabs.personal}</div>
                </div>
              </div>
