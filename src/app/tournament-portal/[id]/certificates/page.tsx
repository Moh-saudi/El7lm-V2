...
const TEMPLATES = [
  { id: 'champion',  label: '🥇 بطل البطولة',        bg: 'linear-gradient(135deg,#78350f,#d97706,#fef3c7)', text: '#fff', accent: '#f59e0b', border: '#f59e0b' },
  { id: 'runner',    label: '🥈 الوصيف (المركز الثاني)', bg: 'linear-gradient(135deg,#1e293b,#475569,#e2e8f0)', text: '#fff', accent: '#94a3b8', border: '#94a3b8' },
  { id: 'third',     label: '🥉 المركز الثالث',        bg: 'linear-gradient(135deg,#431407,#b45309,#fef3c7)', text: '#fff', accent: '#cd7f32', border: '#cd7f32' },
  { id: 'participant', label: '🎖️ شهادة مشاركة',     bg: 'linear-gradient(135deg,#0f172a,#1e3a5f,#dbeafe)', text: '#fff', accent: '#60a5fa', border: '#3b82f6' },
];
...
export default function CertificatesPage() {
  ...
  const title  = customTitle || (tpl.id === 'champion' ? 'شهادة تتويج بطل البطولة' : tpl.id === 'runner' ? 'شهادة المركز الثاني' : tpl.id === 'third' ? 'شهادة المركز الثالث' : 'شهادة مشاركة');
  const body   = customText || `يُشهد بأن ${team?.name || 'الفريق'} قد شارك في بطولة ${tournament?.name || ''} وأبدى روح رياضية عالية وأداءً متميزاً.`;
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
      ...
      <div style={{ background:S.surface, border:`1px solid ${S.border}`, borderRadius:14, padding:'16px 18px', display:'flex', flexDirection:'column', gap:12 }}>
        <div style={{ fontSize:16, fontWeight:800, color:S.text }}>🎖️ إنشاء شهادة</div>

        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          <div>
            <label className="sp-label">الفريق</label>
            <select className="sp-select" value={selTeam} onChange={e=>setSelTeam(e.target.value)}>
              {teams.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}
            </select>
          </div>
          <div>
            <label className="sp-label">نوع الشهادة</label>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={print} className="sp-btn sp-btn-primary">🖨️ طباعة الشهادة</button>
          <button onClick={()=>{ setCustomTitle(''); setCustomText(''); }} className="sp-btn sp-btn-ghost sp-btn-sm">إعادة تعيين</button>
        </div>
      </div>

      {/* Certificate preview */}
      <div style={{ background:S.surface, border:`1px solid ${S.border}`, borderRadius:14, padding:'16px 18px' }}>
        <div style={{ fontSize:14, fontWeight:700, color:S.text, marginBottom:14 }}>معاينة الشهادة</div>
          ...
          <div style={{
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:32 }}>
              ...
              <div style={{ textAlign:'center', flex:1 }}>
                <div style={{ fontSize:13, color:`${tpl.text}80`, fontWeight:600, letterSpacing:2, textTransform:'uppercase', marginBottom:4 }}>CERTIFICATE OF</div>
                <div style={{ fontSize:28, fontWeight:900, color:tpl.accent, lineHeight:1 }}>شهادة تقدير</div>
              </div>
            ...
            <div style={{ textAlign:'center', marginBottom:20 }}>
              ...
              <div style={{ fontSize:36, fontWeight:900, color:tpl.accent, marginBottom:4 }}>{team?.name || 'اسم الفريق'}</div>
            </div>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end' }}>
              <div>
                <div style={{ fontSize:12, color:`${tpl.text}60` }}>البطولة</div>
              ...
              <div style={{ textAlign:'left' }}>
                <div style={{ fontSize:12, color:`${tpl.text}60` }}>تاريخ الإصدار</div>
      ...
      <div style={{ background:S.surface, border:`1px solid ${S.border}`, borderRadius:14, padding:'14px 18px', display:'flex', alignItems:'center', gap:12 }}>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:14, fontWeight:700, color:S.text }}>🖨️ طباعة جماعية</div>
          <div style={{ fontSize:13, color:S.text2, marginTop:2 }}>اطبع شهادات مشاركة لجميع الفرق المقبولة دفعة واحدة</div>
        </div>
        <button
          onClick={() => window.open(`/tournament-portal/${id}/print?type=teams`, '_blank')}
          className="sp-btn sp-btn-ghost sp-btn-sm">
          عرض قائمة الفرق للطباعة
        </button>
      </div>
    </div>
  );
