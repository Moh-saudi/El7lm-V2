...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    if (globeRef.current && globeData && countries && countries.features) {
      globeRef.current
  ...
  const _startAnimation = () => {
    ...
    globeRef.current
      ...
      .arcAltitude((e: any) => (e as any).arcAlt * 1)
    ...
    globeRef.current
      ...
      .ringRepeatPeriod(
        () => (defaultProps.arcTime / data.length)
      );
  };
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      globeRef.current.ringsData(
        globeData.filter((_d, i) => i % 2 === 0 && Math.random() > 0.1)
      );
    }, 2000);
