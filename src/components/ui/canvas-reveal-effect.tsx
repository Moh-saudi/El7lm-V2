...
export const CanvasRevealEffect = ({
    ...
    return (
        <div className={cn("h-full relative bg-white w-full", containerClassName)}>
            ...
            float step_mask = step(intro_offset, u_time * animation_speed_factor);
            float opacity = clamp(step_mask, 0.0, 1.0);
            return opacity;
          `}
                    center={["x", "y"]}
                />
    ...
    return (
        ...
        >
          ...
          void main() {
              ...
              ${center.includes("x")
                        ? "st.x -= abs(floor((u_resolution.x - u_total_size) / 2.0));"
                        : ""
                    }
              ${center.includes("y")
                        ? "st.y -= abs(floor((u_resolution.y - u_total_size) / 2.0));"
                        : ""
                    }
            ...
            switch (uniform.type) {
                ...
                case "uniform3fv":
                    preparedUniforms[uniformName] = {
                        value: (uniform.value as number[][]).reduce(
