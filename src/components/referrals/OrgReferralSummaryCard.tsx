...
export default function OrgReferralSummaryCard({ accountType }: OrgReferralSummaryCardProps) {
  ...
  const rt = (key: string) => t(`sharedComponents.referralSummary.${key}`);
  ...
  return (
    <Card className="p-4 mb-4" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Users className="w-5 h-5 text-purple-600" />
          <h3 className="text-sm font-semibold">{rt('manage')}</h3>
        </div>
      ...
      {loading && <p className="mt-3 text-xs text-gray-500">{rt('loading')}</p>}
      {error && <p className="mt-3 text-xs text-red-600" role="alert">{error}</p>}

      {!loading && !error && (
        <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="p-3 rounded-lg border">
            <div className="text-[11px] text-gray-500">{rt('totalCodes')}</div>
            <div className="text-lg font-bold">{referrals.length}</div>
          </div>
          <div className="p-3 rounded-lg border">
            <div className="text-[11px] text-gray-500">{rt('activeCodes')}</div>
          ...
          <Link href={`/dashboard/${accountType}/referrals`} className="p-3 rounded-lg border hover:bg-gray-50">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[11px] text-gray-500">{rt('createManage')}</div>
                <div className="text-sm font-semibold">{rt('goToPage')}</div>
              </div>
      ...
      {primary && (
        <div className="mt-3 p-3 rounded-lg bg-gray-50 border">
          <div className="text-[11px] text-gray-600 mb-1">{rt('latestCode')}</div>
          <div className="flex flex-wrap items-center gap-2 text-sm">
            ...
            <Button size="sm" variant="outline" onClick={() => copy(primary.referralCode)} className="gap-1 border-purple-200 text-purple-700 hover:bg-purple-50">
              <Copy className="w-4 h-4" />{rt('copyCode')}
            </Button>
            <Button size="sm" variant="outline" onClick={() => copy(primary.inviteLink)} className="gap-1 border-purple-200 text-purple-700 hover:bg-purple-50">
              <Link2 className="w-4 h-4" />{rt('copyLink')}
            </Button>
          </div>
        </div>
      )}
